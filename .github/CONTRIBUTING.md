# Contributing to @takk/bayestruth

Thanks for considering a contribution. This document is the canonical guide for proposing changes to `@takk/bayestruth`.
...
```bash
git commit -s -m "fix(beta): correct the Lanczos peak term in lgamma"
```
...
```bash
git clone https://github.com/davccavalcante/bayestruth.git
cd bayestruth
pnpm install
```
...
Current baseline (verify before opening a PR): **170 tests passing across 20 suites**. Coverage `lines 97.39% / statements 97.48% / functions 100% / branches 93.55%`. Also run `pnpm smoke:dist` and verify on Node 24 before any push.
...
- The JSON snapshot schema of the store or the audit log.
- The CLI flags or subcommands.
- The `PosteriorStore` interface, the conjugate update, or the audit hash-chain construction.
...
CI publishes nothing on merge to `main`. Publishing is a Creator-triggered flow via `git tag v<semver> && git push origin v<semver>` (see [RELEASING.md](./RELEASING.md)).
...
- **Vitest** (`tests/**/*.test.ts`). One file per surface area. Deterministic seeds for randomness; use a deterministic clock for time and decay.
- **Distribution smoke** (`scripts/smoke-dist.mjs`) exercises the compiled ESM and CJS artifacts and spawns the compiled CLI as a single Node process.
- **No external services** in CI. Tests run offline.
